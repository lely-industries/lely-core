...
// TPDO mapping parameter record (0x1a00-0x1bff)
namespace Obj1a00TpdoMapPar {
// sub 0x00 - number of mapped application objects in TPDO
void
Set00NumOfMappedAppObjs(std::unique_ptr<CoObjTHolder>& obj_holder,
                        const co_unsigned8_t number) {
  assert(co_obj_get_idx(obj_holder->Get()) >= 0x1a00u &&
         co_obj_get_idx(obj_holder->Get()) <= 0x1bffu);
