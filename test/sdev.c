...
main(void)
{
	...
	co_dev_t *dev = co_dev_create_from_dcf_file(TEST_SRCDIR "/sdev.dcf");
	tap_assert(dev);

	co_dev_t *sdev = co_dev_create_from_sdev(&test_sdev);
	tap_assert(sdev);
	...
	for (size_t i = 0; i < maxidx; i++) {
		...
		for (size_t i = 0; i < maxsubidx; i++) {
			co_sub_t *sub = co_obj_find_sub(obj, subidx[i]);
			tap_assert(sub);
			co_sub_t *ssub = co_obj_find_sub(sobj, subidx[i]);
			tap_assert(ssub);
