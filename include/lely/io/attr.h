...
/*!
 * Disables flow control for a serial I/O device if \a flow_control is zero, and
 * enables it otherwise. Note that the new device attributes will not take
 ...
 * \a stop_bits is zero, and two otherwise. Note that the new device attributes
 * will not take effect until a call to io_serial_set_attr().
 *
 * \returns 0 on success, or -1 on error. In the latter case, the error number
 * can be obtained with `get_errnum()`.
 *
