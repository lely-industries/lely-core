...
public:
	IOSock(int domain, int type)
		: IOHandle(io_open_socket(domain, type))
	{
		if (!operator bool())
	...
	IOSock(IOSock&& sock) noexcept
		: IOHandle(::std::forward<IOSock>(sock))
	{}
#endif
	...
	int get_domain() const noexcept { return io_sock_get_domain(*this); }
	int get_type() const noexcept { return io_sock_get_type(*this); }
	...
	static int getMaxConn() noexcept { return io_sock_get_maxconn(); }
	...
	int getDebug() const noexcept { return io_sock_get_debug(*this); }
	...
	int getLinger() const noexcept { return io_sock_get_linger(*this); }
	...
	int getRcvBuf() const noexcept { return io_sock_get_rcvbuf(*this); }
	...
	int getSndBuf() const noexcept { return io_sock_get_sndbuf(*this); }
	...
	ssize_t getNRead() const noexcept { return io_sock_get_nread(*this); }
	...
	{
		return io_sock_mcast_block_source(*this, index, &group,
				&source);
	}
	...
	mcastUnblockSource(unsigned int index, const io_addr_t& group,
			const io_addr_t& source) noexcept
	{
		return io_sock_mcast_unblock_source(*this, index, &group,
				&source);
	}
	...
	mcastJoinSourceGroup(unsigned int index, const io_addr_t& group,
			const io_addr_t& source) noexcept
	{
		return io_sock_mcast_join_source_group(*this, index, &group,
				&source);
	}
	...
	mcastLeaveSourceGroup(unsigned int index, const io_addr_t& group,
			const io_addr_t& source) noexcept
	{
		return io_sock_mcast_leave_source_group(*this, index, &group,
				&source);
	}
