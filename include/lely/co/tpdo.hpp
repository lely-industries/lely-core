...
public:
	...
	co_unsigned16_t getNum() const noexcept { return co_tpdo_get_num(this); }
	...
	setInd(F* f) noexcept
	{
		setInd(&c_call<co_tpdo_ind_t, F>::function,
				static_cast<void*>(f));
	}

	template <class T, typename c_mem_fn<co_tpdo_ind_t, T>::type M>
	void
	setInd(T* t) noexcept
	{
		setInd(&c_mem_call<co_tpdo_ind_t, T, M>::function,
				static_cast<void*>(t));
	}
