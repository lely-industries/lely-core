...
LELY_AIO_TIMER_INLINE int aio_clock_getres(const aio_clock_t *clock,
		struct timespec *res);
LELY_AIO_TIMER_INLINE int aio_clock_gettime(const aio_clock_t *clock,
		struct timespec *tp);
LELY_AIO_TIMER_INLINE int aio_clock_settime(aio_clock_t *clock,
		const struct timespec *tp);

LELY_AIO_EXTERN extern const struct aio_clock_realtime {
	const struct aio_clock_vtbl *vptr;
} aio_clock_realtime;

#define AIO_CLOCK_REALTIME	(&aio_clock_realtime.vptr)

LELY_AIO_EXTERN extern const struct aio_clock_monotonic {
	const struct aio_clock_vtbl *vptr;
} aio_clock_monotonic;
...
LELY_AIO_TIMER_INLINE int aio_timer_gettime(const aio_timer_t *timer,
...
LELY_AIO_TIMER_INLINE void aio_timer_submit_wait(aio_timer_t *timer,
		struct aio_task *task);
LELY_AIO_TIMER_INLINE size_t aio_timer_cancel(aio_timer_t *timer,
		struct aio_task *task);

LELY_AIO_EXTERN aio_future_t *aio_timer_async_wait(aio_timer_t *timer,
		aio_loop_t *loop, struct aio_task **ptask);

LELY_AIO_EXTERN void aio_timer_run_wait(aio_timer_t *timer, aio_loop_t *loop);
LELY_AIO_EXTERN void aio_timer_run_wait_until(aio_timer_t *timer,
		aio_loop_t *loop, const struct timespec *abs_time);

LELY_AIO_EXTERN void *aio_timer_alloc(void);
LELY_AIO_EXTERN void aio_timer_free(void *ptr);
LELY_AIO_EXTERN aio_timer_t *aio_timer_init(aio_timer_t *timer,
		clockid_t clockid, aio_exec_t *exec, aio_reactor_t *reactor);
LELY_AIO_EXTERN void aio_timer_fini(aio_timer_t *timer);

LELY_AIO_EXTERN aio_timer_t *aio_timer_create(clockid_t clockid,
		aio_exec_t *exec, aio_reactor_t *reactor);
LELY_AIO_EXTERN void aio_timer_destroy(aio_timer_t *timer);
