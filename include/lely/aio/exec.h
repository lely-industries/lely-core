...
#else
#define AIO_TASK_INIT(exec, func)	{ (exec), (func), 0, NULL }
#endif

LELY_AIO_EXEC_INLINE void aio_exec_run(aio_exec_t *exec, struct aio_task *task);
LELY_AIO_EXEC_INLINE void aio_exec_dispatch(aio_exec_t *exec,
		struct aio_task *task);
LELY_AIO_EXEC_INLINE void aio_exec_post(aio_exec_t *exec,
		struct aio_task *task);
LELY_AIO_EXEC_INLINE void aio_exec_defer(aio_exec_t *exec,
		struct aio_task *task);
...
LELY_AIO_EXTERN void *aio_exec_alloc(void);
LELY_AIO_EXTERN void aio_exec_free(void *ptr);
LELY_AIO_EXTERN aio_exec_t *aio_exec_init(aio_exec_t *exec, aio_loop_t *loop);
LELY_AIO_EXTERN void aio_exec_fini(aio_exec_t *exec);

LELY_AIO_EXTERN aio_exec_t *aio_exec_create(aio_loop_t *loop);
LELY_AIO_EXTERN void aio_exec_destroy(aio_exec_t *exec);
...
LELY_AIO_EXEC_INLINE void
aio_exec_on_task_started(aio_exec_t *exec) { (*exec)->on_task_started(exec); }

LELY_AIO_EXEC_INLINE void
aio_exec_on_task_finished(aio_exec_t *exec) { (*exec)->on_task_finished(exec); }
