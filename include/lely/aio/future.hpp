...
class FutureBase : public detail::CBase<aio_future_t> {
 ...
 public:
  enum class State { WAITING, CANCELED, VALUE, ERROR };
  ...
  void
  RunWaitFor() {
    RunWaitUntil(nullptr);
  }

  void
  RunWaitFor(::std::error_code& ec) {
    RunWaitUntil(nullptr, ec);
  }
...
class Future : public FutureBase {
 ...
 public:
  ...
  T&
  GetValue() const {
    return *static_cast<T*>(FutureBase::GetValue());
  }
