...
class LELY_AIO_EXTERN ClockBase : public detail::CBase<aio_clock_t> {
 public:
  using CBase::CBase;
...
class LELY_AIO_EXTERN BasicClock : public ClockBase {
 public:
  BasicClock() noexcept;
};
...
class LELY_AIO_EXTERN TimerBase : public detail::CBase<aio_timer_t> {
 public:
  using CBase::CBase;
  ...
  void SubmitWait(WaitOperation& op) { SubmitWait(static_cast<aio_task&>(op)); }
  ...
  void RunWaitFor(LoopBase& loop) { RunWaitUntil(loop, nullptr); }
...
class LELY_AIO_EXTERN Timer : public TimerBase {
 public:
