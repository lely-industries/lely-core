...
class UnlockGuard {
 public:
  ...
  /*!
   * Releases ownership of \a m and calls `m.unlock()`. The behavior is
   * undefined if the current thread does not own m.
   */
  explicit UnlockGuard(MutexType& m) : m_(m) { m_.unlock(); }

  /*!
   * Releases ownership of \a m without attempting to unlock it. The behavior is
   * undefined if m current thread already owns m.
   */
  ...
  /*!
   * Acquires ownership of \a m and calls `m.lock()`, where \a m is the mutex
