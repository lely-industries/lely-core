 ...
 */
template <class T>
class Rand : public standard_c_type<typename impl::rand_traits<T>::c_type>,
...
struct rand_traits<uint32_t> {
  ...
  static bool
  eq(const c_type* lhs, const c_type* rhs) {
    return rand_traits<uint64_t>::eq(&lhs->r, &rhs->r) && lhs->x == rhs->x &&
           lhs->n == rhs->n;
  }
