 ...
 * additional \a size bytes. This function may also shrink the buffer if it is
 * larger than required. Although this functions preserves the data already
...
/*!
 * Adjusts the position indicator of a memory buffer by \a offset bytes. The
 * offset will be truncated to a valid range: if negative, -\a offset must not
 * exceed membuf_size(); if positive, \a offset must not exceed
 * membuf_capacity().
 *
 * \param buf    a pointer to a memory buffer.
 ...
 * position indicator given by membuf_size(), and sets the indicator to the end
 * of the requested region. If the requested size is larger than the current
 * capacity, *\a size will be updated with the actual size of the region. If the
 * requested size turns out to be too large or too small, the region can be
 * adjusted with membuf_seek().
 *
 * \param buf  a pointer to a memory buffer.
 * \param size the address of the value containing the requested size. On exit,
 *             *\a size is updated with the actual size.
 *
 * \returns a pointer to a memory region of at least *\a size consecutive
