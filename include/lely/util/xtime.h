 ...
 *
 * @param clock a pointer to a clock.
 * @param flags if the `TIMER_ABSTIME` is set in <b>flags</b>, this function
 *              shall sleep until the value of the specified clock reaches the
 *              absolute time specified by <b>rqtp</b>. If `TIMER_ABSTIME` is
 *              not set, this function shall sleep until the time interval
 *              specified by <b>rqtp</b> has elapsed.
 ...
 *              and `SIGEV_THREAD` are supported for the <b>sigev_notify</b>
 *              member.
 *
 ...
 *               the <b>it_value</b> member contains the time interval until the
 *               first expiration.
 * @param value  a pointer to an `itimerspec` struct containing the period and
 *               expiration of the timer. If the <b>it_value</b> member is zero,
 *               the timer shall be disarmed. If the <b>it_interval</b> member
 *               is non-zero, a periodic (or repetitive) timer is specified.
