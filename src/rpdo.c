		...
		co_unsigned16_t num)
{
	...
	if (__unlikely(!obj_1400 || !obj_1600)) {
		errc = errnum2c(ERRNUM_INVAL);
		goto error_param;
	}
	...
	if (__unlikely(co_rpdo_init_recv(pdo) == -1)) {
		errc = get_errc();
		goto error_init_recv;
	}

	if (__unlikely(co_rpdo_init_timer_swnd(pdo) == -1)) {
		errc = get_errc();
...
co_1400_dn_ind(co_sub_t *sub, struct co_sdo_req *req, void *data)
{
	...
	case 2: {
		...
		if (__unlikely(trans > 0xf0 && trans < 0xfe)) {
			ac = CO_SDO_AC_PARAM_VAL;
			goto error;
		}
	...
	if (!subidx) {
		...
		for (size_t i = 1; i <= n; i++) {
			...
			if (co_type_is_basic(idx) && !subidx) {
				// If the object index is a data type, obtain
				// the size of the dummy entry.
				size = co_type_sizeof(idx);
			} else {
