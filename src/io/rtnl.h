 ...
 *               to the process id only for at most one socket. If \a pid is 0,
 *               the kernel assigns a unique id.
 ...
 * \param payload    the size of the buffer at \a data (in bytes).
 *
 ...
 * that the result can be larger than \a payload, but at most \a payload bytes
 * will be copied to \a data.
 *
 * \see io_rtnl_setattr()
 */
