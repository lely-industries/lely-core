...
Node::Node(aio::TimerBase& timer, aio::CanBusBase& bus,
    ...
    : IoContext(timer, bus, this), Device(dcf_txt, dcf_bin, id, this),
      impl_(new Impl_(this, IoContext::net(), Device::dev())) {}

LELY_COAPP_EXPORT Node::~Node() = default;
...
Node::Reset() {
  ...
  if (impl_->nmt->csInd(CO_NMT_CS_RESET_NODE) == -1)
    throw_errc("Reset");
}

LELY_COAPP_EXPORT void Node::lock() { impl_->lock(); }
LELY_COAPP_EXPORT void Node::unlock() { impl_->unlock(); }
...
LELY_COAPP_EXPORT CONMT* Node::nmt() const noexcept { return impl_->nmt.get(); }
...
Node::Impl_::OnCsInd(CONMT* nmt, uint8_t cs) noexcept {
  if (cs == CO_NMT_CS_START || cs == CO_NMT_CS_ENTER_PREOP) {
    ...
    if (sync)
      sync->setErr<Impl_, &Impl_::OnSyncErr>(this);
    auto time = nmt->getTime();
    if (time)
      time->setInd<Impl_, &Impl_::OnTimeInd>(this);
    auto emcy = nmt->getEmcy();
    if (emcy)
      emcy->setInd<Impl_, &Impl_::OnEmcyInd>(this);
  }
  ...
  if (cs == CO_NMT_CS_START) {
    ...
    for (int i = 1; i <= 512; i++) {
      ...
      if (pdo)
        pdo->setInd<Impl_, &Impl_::OnTpdoInd>(this);
    }
  }
...
void
Node::Impl_::OnRpdoInd(CORPDO* pdo, uint32_t ac, const void* ptr, size_t n)
    noexcept {
  int num = pdo->getNum();
...
void
Node::Impl_::OnTpdoInd(COTPDO* pdo, uint32_t ac, const void* ptr, size_t n)
    noexcept {
