...
LELY_AIO_EXPORT aio_context_t*
ReactorBase::GetContext() const noexcept {
  return aio_reactor_get_context(*this);
}

LELY_AIO_EXPORT PollBase
ReactorBase::GetPoll() const noexcept { return aio_reactor_get_poll(*this); }
...
ReactorBase::Watch(aio_watch& watch, aio_handle_t handle) {
  ...
  if (ec)
    throw ::std::system_error(ec, "Watch");
}
...
LELY_AIO_EXPORT Reactor::Reactor(ContextBase& ctx)
    : ReactorBase(InvokeC("Reactor", aio_reactor_create, ctx)) {}

LELY_AIO_EXPORT Reactor::~Reactor() { aio_reactor_destroy(*this); }
