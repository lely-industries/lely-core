...
struct aio_promise {
	...
	aio_loop_t *loop;
	/*!
	 * A pointer to the default executor for tasks submitted
	 * #with aio_future_submit_wait().
	 */
	aio_exec_t *exec;
	/*!
...
LELY_AIO_EXPORT void aio_loop_free(void *ptr) { free(ptr); }
...
LELY_AIO_EXPORT struct aio_task *
		...
		size_t nfutures, const struct timespec *abs_time)
{
	...
	if (!task && abs_time) {
		proxy->waiting++;
		do {
			int result = cnd_timedwait(&proxy->cond, &loop->mtx,
					abs_time);
...
LELY_AIO_EXPORT aio_promise_t *
aio_promise_create(aio_loop_t *loop, aio_exec_t *exec, aio_dtor_t *dtor,
		void *arg)
{
	assert(loop);
	assert(exec);
...
LELY_AIO_EXPORT enum aio_future_state
...
static aio_future_t *
aio_loop_create_proxy(aio_loop_t *loop,
		aio_future_t *const *futures, size_t nfutures)
{
	assert(loop);
	assert(futures || !nfutures);
